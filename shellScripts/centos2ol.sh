...
for repo in base.repos.iter_enabled():
  print(repo.id)
")
enabled_repos=${enabled_repos:=appstream} ## Ensure that $enabled_repos at least includes 'appstream'
        ;;
    *)
...
for repo in base.repos.listEnabled():
  print repo
")
enabled_repos=${enabled_repos:=updates} ## Ensure that $enabled_repos at least includes 'updates'       
;;
...
######################################################################################################################
## RHEL (all versions?) release packages do not contain *.repo files and will crash the script here.
## Correcting that behavior and bypassing repo backup for RHEL systems

if [[ $old_release =~ ^redhat-release-.* ]]; then write-information "Bypassing repo backup on unsubscribed RHEL system..."; else
...
case "$os_version" in
    6*)
	    ...
	    ['updates']="REPO ol6_latest,ol6_UEK_latest"        
)
        ;;
    7*)
        declare -A repositories=(
            [base-debuginfo]="REPO https://oss.oracle.com/ol7/debuginfo/"
            ['updates']="REPO ol7_latest,ol7_optional_latest,ol7_UEKR6"    
            ['ol7_latest']="REPO ol7_latest,ol7_optional_latest,ol7_UEKR6"    
